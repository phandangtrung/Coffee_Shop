  ...
  const getshipperById = async (req, res, next) => {
    ...
    try {
      ...
      const error = new HttpError(
        'Something went wrong, could not find a shipper.',500);
      return next(error);
    }
  
    if (!shippers) {
      const error = new HttpError(
        'Could not find a shipper for the provided id.',404);
      return next(error);
    }
...
module.exports = {createShipper, updateShipperById, deleteShipperById, getAllShipper, getshipperById }